...
namespace MxRendering::Resources::Loaders
{
...
using namespace MxRendering::Resources::Loaders;
...
namespace
{
    ...
    static HRESULT CreateTextureFromWIC12(
        ...
        ComPtr<ID3D12Resource>& textureUploadHeap)
    {
        ...
        std::unique_ptr<uint8_t[]> decodedData(new (std::nothrow) uint8_t[imageSize]);
        if (!decodedData)
            return E_OUTOFMEMORY;
		...
		else
		{
			...
			else
			{
				...
				subResourceData.pData = decodedData.get();
				subResourceData.RowPitch = static_cast<LONG>(rowPitch);
				subResourceData.SlicePitch = static_cast<LONG>(imageSize);

				// Use Heap-allocating UpdateSubresources implementation for variable number of subresources (which is the case for textures).
				UpdateSubresources<1>(cmdList, texture.Get(), textureUploadHeap.Get(), 0, 0, num2DSubresources, &subResourceData);
...
HRESULT MxRendering::Resources::Loaders::LoadWICTextureFromMemory(
...
_Use_decl_annotations_
HRESULT MxRendering::Resources::Loaders::CreateWICTextureFromMemory(
    ID3D12Device* d3dDevice,
    const uint8_t* wicData,
    size_t wicDataSize,
    ID3D12Resource** texture,
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
HRESULT MxRendering::Resources::Loaders::LoadWICTextureFromMemoryEx(
    ID3D12Device* d3dDevice,
    const uint8_t* wicData,
    size_t wicDataSize,
    size_t maxsize,
...
_Use_decl_annotations_
HRESULT MxRendering::Resources::Loaders::CreateWICTextureFromMemoryEx(
    ID3D12Device* d3dDevice,
    const uint8_t* wicData,
    size_t wicDataSize,
    size_t maxsize,
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
HRESULT MxRendering::Resources::Loaders::LoadWICTextureFromFile(
    ID3D12Device* d3dDevice,
...
_Use_decl_annotations_
HRESULT MxRendering::Resources::Loaders::CreateWICTextureFromFile(
    ID3D12Device* d3dDevice,
    const wchar_t* fileName,
    ID3D12Resource** texture,
    bool generateMips,
...
HRESULT MxRendering::Resources::Loaders::CreateWICTextureFromFile12(
    _In_ ID3D12Device* d3dDevice,
    ...
    _In_ size_t maxsize /*= 0*/)
{
	...
	hr = CreateTextureFromWIC12(d3dDevice, cmdList, frame.Get(), D3D12_RESOURCE_FLAG_NONE,
		WIC_LOADER_DEFAULT, maxsize, false, texture, textureUploadHeap);
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
HRESULT MxRendering::Resources::Loaders::LoadWICTextureFromFileEx(
    ID3D12Device* d3dDevice,
    const wchar_t* fileName,
    size_t maxsize,
    D3D12_RESOURCE_FLAGS resFlags,
...
_Use_decl_annotations_
HRESULT MxRendering::Resources::Loaders::CreateWICTextureFromFileEx(
    ID3D12Device* d3dDevice,
    const wchar_t* fileName,
    size_t maxsize,
    D3D12_RESOURCE_FLAGS resFlags,
