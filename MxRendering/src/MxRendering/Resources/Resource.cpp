...
#include "MxRendering/Common/CommonVertexFormat.h"
...
HRESULT MxRendering::Resources::CreateShaderResourceViewFromFile(const wchar_t* szFileName, Texture* textureData)
{
	if (!szFileName)
		return E_INVALIDARG;

	auto d3dDevice = MxRendering::Core::MxRenderer::GetRenderer()->GetDevice().Get();
	auto commderList = MxRendering::Core::MxRenderer::GetRenderer()->GetCommandList().Get();
	HRESULT hr;
	WCHAR ext[_MAX_EXT];
...
HRESULT MxRendering::Resources::LoadModelFromFile(const char* szFileName)
{
	HRESULT hr = S_OK;
	auto d3dDevice = MxRendering::Core::MxRenderer::GetRenderer()->GetDevice().Get();
	auto commderList = MxRendering::Core::MxRenderer::GetRenderer()->GetCommandList().Get();
	...
	if (result == cgltf_result_success)
	{
		...
		MxRendering::Core::MxRenderer::GetRenderer()->SaveMesh(geo);
		...
		for (int i = 0; i < data->materials_count; ++i)
		{
			...
			if (materialPointer->has_pbr_metallic_roughness)
			{
				if (materialPointer->pbr_metallic_roughness.base_color_texture.texture)
				{
					...
					MxRendering::Core::MxRenderer::GetRenderer()->SaveTexturePath("Default_albedo", AnsiToWString(filename.str()));
				}
				...
				if (materialPointer->pbr_metallic_roughness.metallic_roughness_texture.texture)
				{
					...
					std::stringstream filename;
					filename << "Assets/GltfModel/" + (std::string)name + "";
					MxRendering::Core::MxRenderer::GetRenderer()->SaveTexturePath("metallicRoughnessTexture", AnsiToWString(filename.str()));
				}
			}
			...
			if (materialPointer->normal_texture.texture)
			{
				...
				std::stringstream filename;
				filename << "Assets/GltfModel/" + (std::string)name + "";
				MxRendering::Core::MxRenderer::GetRenderer()->SaveTexturePath("Default_normal", AnsiToWString(filename.str()));
			}
			...
			if (materialPointer->emissive_texture.texture)
			{
				...
				std::stringstream filename;
				filename << "Assets/GltfModel/" + (std::string)name + "";
				MxRendering::Core::MxRenderer::GetRenderer()->SaveTexturePath("Default_emissive", AnsiToWString(filename.str()));
			}
		}
